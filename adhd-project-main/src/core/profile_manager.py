...
class ProfileManager:
    ...
    def create_user_structure(self, user, base_path="data"): 
    ...
    def load_user(self, user_id):
        ...
        user = User(int(uid_str), data["name"], data["password"])
        user.stats = data["stats"]

        user_dir = data["name"] + "_" + uid_str
        config_path = os.path.join(self.data_dir, user_dir, "config.json")
        config = self.json_utils.load_json(config_path)
        if not config:
            raise ValueError(f"Config missing for user {user.username}")
